...
#define MAX_DMA_TRANSFER 0XFFFF
// Função para enviar dados 16bits para o display
void ST7796_SendDataDMA(TDisplayST7796 *_display, uint16_t *_buffer, uint32_t _size) {
  ...
  for (uint32_t offset = 0; offset < _size; offset += MAX_DMA_TRANSFER) {
    _display->dmaBusy = true;
    
    uint32_t restante = _size - offset;
    uint16_t calcSize = (restante > MAX_DMA_TRANSFER) ? MAX_DMA_TRANSFER : (uint16_t)restante;
    HAL_SPI_Transmit_DMA(_display->spi, (uint8_t *)&_buffer[offset], calcSize);
    //HAL_Delay(1000);
    while (_display->dmaBusy); // Esperar até que o DMA esteja livre
  }
  //HAL_GPIO_WritePin(ST7796_CS_GPIO_Port, ST7796_CS_Pin, GPIO_PIN_SET);
}

static void ST7796_SendBufferWindow(TDisplayST7796 *_display, uint8_t iWindow) {
  uint16_t sizeBuffer = _display->bufferManager->windows[iWindow].width * _display->bufferManager->windows[iWindow].height;
  HAL_GPIO_WritePin(ST7796_DC_GPIO_Port, ST7796_DC_Pin, GPIO_PIN_SET); // DC = 1 para dados
  HAL_GPIO_WritePin(ST7796_CS_GPIO_Port, ST7796_CS_Pin, GPIO_PIN_RESET); // CS = 0 para selecionar
  ST7796_SendDataDMA(_display,_display->bufferManager->windows[iWindow].buffer, sizeBuffer);
}

void ST7796_SendBuffer(TDisplayST7796 *_display) {
  for(uint8_t i = 0; i < _display->bufferManager->windowCount; i++){
    if(_display->bufferManager->windows[i].isPaint){
      ST7796_SendBufferWindow(_display, i);
      _display->bufferManager->windows[i].isPaint = false; // Marcar a janela como processada
    }
  }
}
...
{
  _display->spi = _hspi;
  _display->dmaBusy = false;
  _display->temTranparencia = false;
  _display->transparenciaCor = 0x0000;
  _display->backgroundCor = 0xFFFF;
  _display->bufferManagerAtivo = false;
  _display->bufferManager = NULL;
...
static void j3_ST7796_drawBitmapDMA(TDisplayST7796 *_display, uint16_t _x, uint16_t _y, uint16_t _largura, uint16_t _altura, const uint16_t *_bitmap)
{
  uint16_t x2;
  uint16_t y2;
  ...
  while( contBitmap < (_largura * _altura)){
    flagEnviouDMA = false;
      if (_display->temTranparencia && (_bitmap[contBitmap] == _display->transparenciaCor)){
	       j3memBuffer[contBuffer] = _display->backgroundCor;
      }else{
	      j3memBuffer[contBuffer] = _bitmap[contBitmap];
      }

    if(contBuffer >= TAM_BUFFER ){

      ST7796_SendDataDMA(_display, (uint16_t *)j3memBuffer, (contBuffer + 1));
...
static void j3_ST7796_drawBitmapBuffer(TDisplayST7796 *_display, uint16_t _x, uint16_t _y, uint16_t _largura, uint16_t _altura, const uint16_t *_bitmap)
{
  uint16_t x2;
  uint16_t y2;
  uint32_t contBitmap = 0;
  ...
  j3_bufferManager_addWindow(_display->bufferManager, _x, _y, _largura, _altura);

  _display->bufferManager->windows[_display->bufferManager->windowCount - 1].isPaint = true;
  while( contBitmap < (_largura * _altura)){
      if (_display->temTranparencia && (_bitmap[contBitmap] == _display->transparenciaCor)){
	       _display->bufferManager->windows[_display->bufferManager->windowCount - 1].buffer[contBuffer] = _display->backgroundCor;
      }else{
	      _display->bufferManager->windows[_display->bufferManager->windowCount - 1].buffer[contBuffer] = _bitmap[contBitmap];
      }
...
void j3_ST7796_drawBitmap(TDisplayST7796 *_display, uint16_t _x, uint16_t _y, uint16_t _largura, uint16_t _altura, const uint16_t *_bitmap){
  if(_display->bufferManagerAtivo){
    j3_ST7796_drawBitmapBuffer(_display, _x, _y, _largura, _altura, _bitmap);
  }
  else{
    j3_ST7796_drawBitmapDMA(_display, _x, _y, _largura, _altura, _bitmap);
  }
}
...
void j3_ST7796_fillBackground(TDisplayST7796 *_display)
{
  ...
  //j3_ST7796_fillScreen(_display, _display->backgroundCor);
}
...
void j3_ST7796_fillScreenDMA(TDisplayST7796 *_display, uint16_t color) {
  ...
  uint16_t *buf16 = (uint16_t *)j3memBuffer;
  uint32_t max_pixels_no_buffer = BUFFER_SIZE / 2; // Quantidade de PIXELS que cabem no buffer
  // Preenche o buffer todo com a cor desejada (Sem bit shift, apenas atribuição direta!)
  for(uint32_t i = 0; i < max_pixels_no_buffer; i++) {
    buf16[i] = color; 
  }

  // 3. Envia os dados para a tela
  uint32_t total_pixels = ST7796_WIDTH * ST7796_HEIGHT;
  uint32_t pixels_enviados = 0;

  while(pixels_enviados < total_pixels) {
    
    // Calcula quantos pixels ainda faltam enviar
    uint32_t pixels_restantes = total_pixels - pixels_enviados;
    
    // Se faltar mais do que o buffer aguenta, manda o buffer cheio. 
    // Se faltar menos (o finalzinho da tela), manda só o que falta.
    uint32_t pixels_para_enviar_agora = (pixels_restantes > max_pixels_no_buffer) ? max_pixels_no_buffer : pixels_restantes;
   
    ST7796_SendDataDMA(_display, buf16, pixels_para_enviar_agora);

    pixels_enviados += pixels_para_enviar_agora;
  }
  // Após enviar tudo, o CS é liberado na função de callback do DMA
  HAL_GPIO_WritePin(ST7796_CS_GPIO_Port, ST7796_CS_Pin, GPIO_PIN_SET);
}
...
void j3_ST7796_fillWindow(TDisplayST7796 *_display, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t _cor)
{
  ...
  while( contScreen <  ( ((x2 - x1) + 1) * ((y2 - y1) + 1)) ){
      flagEnviouDMA = false;
      j3memBuffer[contBuffer] = _cor;

      if(contBuffer >= 65535){
	  ST7796_SendDataDMA(_display, (uint16_t *)j3memBuffer, contBuffer / 2);
	  while(_display->dmaBusy);
	  flagEnviouDMA = true;
	  contBuffer = 0;
      }else{
